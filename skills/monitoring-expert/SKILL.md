...
description: >
  서비스 관측성과 성능 진단. TRIGGER when the user sets up Prometheus/Grafana, writes structured logging or
  alerting rules, instruments distributed tracing, debugs production issues from
  logs/metrics/traces, runs load tests (k6, Artillery), or profiles CPU/memory and plans capacity.
license: MIT
metadata:
