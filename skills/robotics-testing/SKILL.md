...
description: >
  로봇 소프트웨어 테스트 전략. TRIGGER when the user writes tests for ROS nodes (pytest, launch_testing),
  builds simulation or hardware-in-the-loop tests, mocks sensors and hardware, sets up
  deterministic replay or regression tests for robot behaviors, or runs robotics CI.
---
