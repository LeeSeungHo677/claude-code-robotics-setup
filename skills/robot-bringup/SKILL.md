...
name: robot-bringup
description: >
  로봇 온보드 컴퓨터에서 ROS 2 스택을 부팅부터 띄우고 운영한다. TRIGGER when the user sets ROS 2 to start on boot, writes
  systemd units for launch, composes layered launch files, adds watchdog or ordered startup with
  health checks, writes udev rules for cameras/serial devices, or debugs boot race conditions and
  graceful shutdown.
---
