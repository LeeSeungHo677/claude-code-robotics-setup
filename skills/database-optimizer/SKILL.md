...
description: >
  관계형 DB 쿼리·성능 최적화. TRIGGER when the user investigates slow queries, reads execution plans,
  designs indexes, rewrites queries, tunes configuration, partitions tables, or resolves lock
  contention on PostgreSQL/MySQL.
license: MIT
metadata:
