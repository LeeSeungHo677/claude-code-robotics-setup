...
description: >
  ROS 2 를 웹과 잇는다 — 대시보드, 원격 관제, 영상 스트리밍. TRIGGER when the user builds a robot dashboard or browser
  UI, bridges ROS 2 with rosbridge/roslibjs/FastAPI/Flask, streams telemetry over WebSocket,
  streams camera video (MJPEG/WebRTC/RTSP), exposes ROS 2 services as REST, or runs an async web
  framework alongside the ROS 2 executor.
---
